...
int main(int, char**)
{
	...
	image->Load("../Resources/flower.bmp", 126);
	image->Flip();
	...
	while (!quit)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			framebuffer->DrawRect(rand() % renderer->width, rand() % renderer->height, 50, 50, { (uint8_t)((rand() % 2) * 255), (uint8_t)((rand() % 2) * 255), (uint8_t)((rand() % 2) * 255), 64 });
		}
