...
void Framebuffer::DrawCircleOctants(int cx, int cy, int x, int y, const color_t& color)
{
    ...
    DrawLine(cx - x, cy + y, cx + y, cy + x, color);
    DrawLine(cx - x, cy - y, cx + y, cy - x, color);
...
void Framebuffer::DrawCubicCurve(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int steps, const color_t& color)
{
    ...
    for (int i = 0; i < steps; i++)
    {
        ...
        float a3 = (float)pow((1.0f - t3), 3.0f);
        float b3 = 3.0f * (float)pow((1.0f - t3), 2.0f) * t3;
        float c3 = 3.0f * (1.0f - t3) * (float)pow(t3, 2.0f);
        float d3 = (float)pow(t3, 3.0f);
        ...
        int sx3 = (int)(a3 * x1 + b3 * x2 + c3 * x3 + d3 * x4);
        int sy3 = (int)(a3 * y1 + b3 * y2 + c3 * y3 + d3 * y4);
        
        DrawLine(sx1, sy1, sx2, sy2, color);
        DrawLine(sx2, sy2, sx3, sy3, color);
    }
}
